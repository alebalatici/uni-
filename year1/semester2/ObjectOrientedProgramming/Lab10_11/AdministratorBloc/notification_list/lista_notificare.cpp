...
{
    if (lista_notificari.empty())
    {
        throw ListaNotificareException("Lista de notificari este deja goala\n");
    }
...
{
    if (number > int(original_notificari.size()))
    {
...
void ListaNotificare::export_to_html(const std::string &filename) const
{
    if (lista_notificari.empty())
    {
        throw ListaNotificareException("Lista de notificari este goala\n");
    }
